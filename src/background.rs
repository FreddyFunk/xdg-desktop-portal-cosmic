...
impl Background {
    ...
    /// Write `desktop_entry` to path `autostart_entry`.
    ///
...
impl Background {
    ...
    async fn notify_background(
        ...
        // Read config via a watch receiver rather than `&mut self`: a `&mut self` interface
        // method holds the interface lock and can deadlock zbus.
        ...
        match config.default_perm {
            ...
            PermissionDialog::Ask => {
                ...
                // Wrap in a Request so that the frontend can cancel the prompt via
                // `org.freedesktop.impl.portal.Request.Close`; otherwise the method would
                // block forever and leak the dialog window if the request is withdrawn.
                Request::run(
                    connection,
                    &handle,
                    move || async move {
...
pub(crate) fn view(portal: &CosmicPortal, id: window::Id) -> cosmic::Element<'_, Msg> {
    let args = portal.background_prompts.get(&id);
    ...
    widget::dialog()
        ...
        .secondary_action(
            widget::button::standard(fl!("allow-once")).on_press(Msg::Response {
                id,
...
/// Register Background dialog args for a specific window
pub fn update_args(portal: &mut CosmicPortal, args: Args) -> cosmic::Task<crate::app::Msg> {
    ...
    // Open a layer surface for the dialog.
...
pub fn update_msg(portal: &mut CosmicPortal, msg: Msg) -> cosmic::Task<crate::app::Msg> {
    match msg {
        Msg::Response { id, choice } => {
            ...
            // Return result to the portal handler
...
/// Close a background prompt that the frontend withdrew via `Request.Close`.
///
/// The pending D-Bus method has already been aborted and will return `Cancelled`, so we
/// only need to drop the dialog state and tear down its surface.
